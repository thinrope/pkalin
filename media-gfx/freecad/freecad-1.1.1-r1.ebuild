...
if [[ ${PV} == *9999* ]]; then
	...
	#Download the AddonManager Submodule too
	ADDON_MGR_COMMIT="937b6877239dc78ef59eeefe8099e5f14243eda1"
	SRC_URI="
	https://github.com/FreeCAD/FreeCAD/archive/refs/tags/${PV}.tar.gz -> ${P}.tar.gz
	https://github.com/FreeCAD/AddonManager/archive/${ADDON_MGR_COMMIT}.tar.gz \
		-> FreeCAD-AddonManager-${ADDON_MGR_COMMIT}.tar.gz
	"
	KEYWORDS="~amd64"
	S="${WORKDIR}/FreeCAD-${PV}"
fi
...
PATCHES=(
	...
	"${FILESDIR}/${PN}-1.1.1-pycxx-no-pkgconf.patch"
)
...
src_configure() {
	...
	local mycmakeargs=(
		...
		-DFREECAD_USE_EXTERNAL_PYCXX="yes"
